...
async function main() {
  ...
  // --- BOOTSTRAP MATERIALIZED VIEW FOR AI ---
  console.log('Bootstrapping AI Materialized View...');
  const BOOTSTRAP_VIEW_SQL = `
  ...
  const BOOTSTRAP_INDEX_SQL = `CREATE UNIQUE INDEX IF NOT EXISTS "idx_requisition_summary_id" ON "RequisitionSystemWideSummary" (id);`;

  await prisma.$executeRawUnsafe(BOOTSTRAP_VIEW_SQL);
  await prisma.$executeRawUnsafe(BOOTSTRAP_INDEX_SQL);
  console.log('AI Materialized View bootstrapped.');
