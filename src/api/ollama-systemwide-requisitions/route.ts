...
console.log('[ollama-systemwide-requisitions] module loaded');

/**
 * GET: returns full detailed system-wide requisitions dataset.
 * POST: accepts a simple user prompt (non-technical) and returns the
 * full dataset plus a plain-language `ollama` prompt that can be sent to
 * an LLM (ollama) to produce human-friendly answers.
 */
...
export async function POST(request: Request) {
    try {
        console.log('[ollama-systemwide-requisitions] POST handler called');
        const body = await request.json().catch(() => ({}));
        console.log('[ollama-systemwide-requisitions] request body length:', JSON.stringify(body || {}).length);
        ...
        // Build a simple, non-technical prompt wrapper for Ollama/system-wide
        ...
        // Provide the full data context to the model. The assistant MUST only use
        // the data provided below and must not invent facts or call external
        // knowledge. If information is missing, state that it is missing.
        ...
        // Send to Ollama (server-side) so the client receives a ready-made humanized answer
        try {
            ...
            if (!res.ok) {
                ...
                // return prompt and data so client can still use it
