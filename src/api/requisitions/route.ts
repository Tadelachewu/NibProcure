...
export async function GET(request: Request) {
  ...
  const isOpenTenderParam = searchParams.get('isOpenTender');
  ...
  if (token) {
    try {
      ...
      // Token might be invalid or expired, proceed without user context for public routes
    }
  }
  ...
  try {
    ...
    if (isOpenTenderParam === 'true') {
      ...
      // preserve the base whereClause before adding vendor-specific ORs so
      // we can fetch a broader set and apply JS-side filtering for
      // per-item award details (JSON[] matches).
    ...
    // Prisma cannot filter on JSON[] with the desired operator reliably across
    // providers. Instead, fetch matching requisitions (without JSON array
    // filtering), apply the per-item JSON filter in JS for vendor views, and
    // then paginate the results. This keeps behavior correct while avoiding
    // invalid Prisma JSON operators.
    const fetchWhere = (forVendor === 'true' && baseWhere) ? baseWhere : whereClause;
    ...
    // If this is a vendor view, include requisitions that match per-item
    // award details for the vendor. The original Prisma filter attempted to
    // match JSON arrays; we perform that check here in JS to avoid invalid
    // Prisma operators.
    let filteredRequisitions = allRequisitions;
    if (forVendor === 'true' && userPayload?.vendorId) {
      const vid = userPayload.vendorId;
      const now = new Date();
      filteredRequisitions = allRequisitions.filter(r => {
        // 1) Any quotation already submitted by vendor
        if ((r.quotations || []).some(q => q.vendorId === vid)) return true;

        // 2) Any per-item award detail references this vendor
        if ((r.items || []).some(item =>
          (item.perItemAwardDetails || []).some((d: any) => d && d.vendorId === vid)
        )) return true;

        // 3) Requisition is accepting quotes, deadline in future, vendor allowed and hasn't quoted
        if (r.status === 'Accepting_Quotes' && r.deadline && new Date(r.deadline) > now) {
          const allowed = r.allowedVendorIds || [];
          const allowedEmpty = Array.isArray(allowed) && allowed.length === 0;
          const allowedHas = Array.isArray(allowed) && allowed.includes(vid);
          const hasQuoted = (r.quotations || []).some(q => q.vendorId === vid);
          if ((allowedEmpty || allowedHas) && !hasQuoted) return true;
        }
    ...
    const totalCount = filteredRequisitions.length;
    const requisitions = filteredRequisitions.slice(skip, skip + limit);
...
export async function PATCH(
  ...
  try {
    ...
    const { id, status, userId, comment, isOpenTender } = body;
