...
export async function PATCH(
  ...
  try {
    ...
    if (!(actor.roles as string[]).includes('Procurement_Officer')) {
        return NextResponse.json({ error: 'Unauthorized: Only Procurement Officers can verify vendors.' }, { status: 403 });
    }
