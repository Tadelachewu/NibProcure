...
import { ApprovalSummaryDialog } from '@/components/approval-summary-dialog';
...
export default function AwardReviewsPage() {
  ...
  const submitAction = async () => {
    ...
    try {
      // ** MODIFICATION: Emit an event when reject is clicked **
      // This allows our new, independent module to handle the logic.
      if (actionType === 'reject') {
        console.log(`[EVENT] Emitting awardRejected for ${selectedRequisition.id}`);
        workflowEventBus.emit('awardRejected', {
          requisitionId: selectedRequisition.id,
          userId: user.id,
          comment: justification,
        });
        // We can still let the original (broken) API call go through,
        // or comment it out. The new system will now handle it.
      }
      ...
      toast({
        ...
        description: `Your decision for requisition ${selectedRequisition.id} has been recorded.`,
      });
