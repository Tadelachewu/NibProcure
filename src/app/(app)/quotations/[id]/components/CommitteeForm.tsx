...
const MemberSelection = ({ type }: { type: 'financial' | 'technical' }) => {
    ...
    const committeeMembers = allUsers.filter(u => (u.roles as any[]).some(r => r.name === 'Committee_Member'));
    const otherType = type === 'financial' ? 'technical' : 'financial';
    const otherCommitteeIds = new Set(useFormContext().watch(`${otherType}CommitteeMemberIds`));
