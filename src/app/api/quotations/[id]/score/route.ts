...
export async function POST(
    ...
    try {
        ...
        // Enforce scoring/compliance deadline: block submissions after the configured scoringDeadline
        if (requisition.scoringDeadline) {
            const now = new Date();
            const deadline = new Date(requisition.scoringDeadline);
            if (now > deadline) {
                return NextResponse.json({ error: 'Scoring/compliance deadline has passed. You can no longer submit checks.' }, { status: 403 });
            }
        }
