  ...
  try {
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
        ...
        if (action === 'accept') {
            ...
            // This logic is now handled by the payment endpoint to prevent premature closing.
            // We only update to PO_Created if all awards are resolved.
            ...
            if (isPerItemAward) {
                allAwardsResolved = !updatedRequisitionAfterPO.items.some(item =>
                    (item.perItemAwardDetails as PerItemAwardDetail[] | undefined)?.some(d => ['Awarded', 'Pending_Award', 'Standby'].includes(d.status))
                );
            } else {
                allAwardsResolved = !updatedRequisitionAfterPO.quotations.some(q => ['Awarded', 'Pending_Award', 'Standby'].includes(q.status));
            }

            if (allAwardsResolved) {
                console.log(`[RESPOND-AWARD] All awards for Req ${requisition.id} have been resolved. Updating status to PO_Created.`);
                await tx.purchaseRequisition.update({
                    where: { id: requisition.id },
                    data: { status: 'PO_Created' }
                });
            } else {
                console.log(`[RESPOND-AWARD] Not all awards for Req ${requisition.id} have been resolved yet. Status remains active.`);
            }
