...
export async function POST(request: Request) {
  try {
    ...
    const transactionResult = await prisma.$transaction(async (tx) => {
        // Generate a new ID that will be used for both the requisition and its transaction tracking.
        const newRequisitionId = `REQ-${Date.now()}`; // Or use a more robust UUID library in production

        const newRequisition = await tx.purchaseRequisition.create({
            data: {
                id: newRequisitionId,
                transactionId: newRequisitionId,
