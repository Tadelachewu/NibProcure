...
import { CalendarIcon, Upload, Calculator } from 'lucide-react';
...
export const AwardCenterDialog = ({
    ...
    const overallWinner = useMemo(() => {
        if (!singleVendorResults || singleVendorResults.length === 0) return null;
        return singleVendorResults[0];
    }, [singleVendorResults]);

    const standbyVendors = useMemo(() => {
        if (!singleVendorResults || singleVendorResults.length < 2) return [];
        return singleVendorResults.slice(1, 3);
    }, [singleVendorResults]);
    ...
    const handleConfirmAward = async () => {
        ...
        awards[overallWinner.vendorId] = { 
            ...
            items: overallWinner.itemBids.map((bid: any) => ({
                requisitionItemId: bid.requisitionItemId,
                quoteItemId: bid.championBid.quoteItemId
            }))
        };
                            ...
                            <TableBody>
                                {overallWinner && (
                                    <TableRow className="bg-green-500/10">
                                        ...
                                        <TableCell className="font-mono">{overallWinner.finalVendorScore?.toFixed(2)}</TableCell>
                                        <TableCell className="font-mono">{quotations.find(q => q.vendorId === overallWinner.vendorId)?.totalPrice.toLocaleString()} ETB</TableCell>
                                    </TableRow>
                                )}
                                {standbyVendors.map((vendor, index) => (
                                     <TableRow key={vendor.vendorId}>
                                        ...
                                        <TableCell className="font-mono">{vendor.finalVendorScore?.toFixed(2)}</TableCell>
                                        <TableCell className="font-mono">{quotations.find(q => q.vendorId === vendor.vendorId)?.totalPrice.toLocaleString()} ETB</TableCell>
                                    </TableRow>
                                ))}
