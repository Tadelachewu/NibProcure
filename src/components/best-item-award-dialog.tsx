...
export const BestItemAwardDialog = ({
    ...
    const handleConfirmAward = async () => {
        ...
        onFinalize('item', awards, awardResponseDeadline, minuteType, minuteFilePath);
        onClose();
    }
            ...
            <DialogContent className="max-w-4xl">
                ...
                <div className="space-y-4 pt-4 border-t">
                    ...
                    <RadioGroup value={minuteType} onValueChange={(value: 'system' | 'manual') => setMinuteType(value)} className="grid grid-cols-2 gap-4">
                        <div>
                            <RadioGroupItem value="system" id="item-minute-system" className="peer sr-only" />
                            <Label htmlFor="item-minute-system" className="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary">
                                <FileText className="mb-3 h-6 w-6" />
                                System Generated
                            </Label>
                        </div>
                        <div>
                            <RadioGroupItem value="manual" id="item-minute-manual" className="peer sr-only" />
                            <Label htmlFor="item-minute-manual" className="flex flex-col items-center justify-between rounded-md border-2 border-muted bg-popover p-4 hover:bg-accent hover:text-accent-foreground peer-data-[state=checked]:border-primary [&:has([data-state=checked])]:border-primary">
                    ...
                    {minuteType === 'manual' && (
                        <div className="pt-2">
                            <Label htmlFor="manual-minute-file-item">Upload Minute Document (PDF)</Label>
                            <Input id="manual-minute-file-item" type="file" accept=".pdf" onChange={(e) => setMinuteFile(e.target.files?.[0] || null)} />
                        </div>
                    )}
                </div>
