...
import { AlertTriangle, ChevronsRight, ChevronRight, ChevronLeft, ChevronsLeft, PackageX, CheckCircle, Clock } from 'lucide-react';
...
const PendingPaymentsView = ({ pendingPOs }: { pendingPOs: PurchaseOrder[] }) => {
    const [currentPage, setCurrentPage] = useState(1);
    const totalPages = Math.ceil(pendingPOs.length / 5);
    const paginatedPOs = useMemo(() => {
        const startIndex = (currentPage - 1) * 5;
        return pendingPOs.slice(startIndex, startIndex + 5);
    ...
    return (
        <Card>
            <CardHeader>
                ...
                <CardDescription>
                    Purchase Orders with goods received but invoices not fully paid.
                </CardDescription>
            </CardHeader>
            <CardContent>
                <div className="border rounded-md">
...
export function GoodsReceiptForm() {
    ...
    const { openablePOs, disputedPOs, completedPOs, pendingPOs } = useMemo(() => {
        ...
        allPurchaseOrders.forEach(po => {
            ...
            const isPaid = po.invoices?.length ? po.invoices.every(i => i.status === 'Paid') : false;
            ...
            if (isDisputed) {
                ...
                completed.push(po);
            } else if (hasReceipts && !isPaid) {
                // Goods received but invoices not fully paid
                pending.push(po);
        ...
        return { openablePOs: openable, disputedPOs: disputed, completedPOs: completed, pendingPOs: pending };
    }, [allPurchaseOrders]);
    ...
    return (
        <div className="space-y-8">
            ...
            {viewMode === 'pending' && <PendingPaymentsView pendingPOs={pendingPOs} />}
