...
function AddInvoiceForm({ onInvoiceAdded }: { onInvoiceAdded: () => void }) {
    ...
    useEffect(() => {
        const fetchData = async () => {
            ...
            // Only allow invoice creation for POs that have a goods receipt logged
            setPurchaseOrders(poData.filter((po: PurchaseOrder) => po.status !== 'Cancelled' && (po.receipts || []).length > 0));
        };
        fetchData();
    }, []);
