...
import { Loader2, Check, X, Eye, FileBarChart2, Info, Printer, ShieldQuestion, User, History } from 'lucide-react';
import { useForm } from 'react-hook-form';
import { zodResolver } from '@hookform/resolvers/zod';
import * as z from 'zod';
import { Form, FormControl, FormField, FormItem, FormLabel, FormMessage } from '@/components/ui/form';
import { PurchaseRequisition, Quotation, AuditLog as AuditLogType, EvaluationCriterion, Minute } from '@/lib/types';
            ...
            <Card>
                ...
                <CardContent className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        ...
                        <div><Label>Created</Label><p>{requisition.createdAt ? format(new Date(requisition.createdAt), 'PP') : 'N/A'}</p></div>
                    </div>
                     <div>
                    ...
                    <ScrollArea className="h-40">
                         <div className="relative pl-6">
                            ...
                            <div key={log.id} className="relative mb-6">
                                <div className="absolute -left-0 top-1.5 flex h-6 w-6 items-center justify-center rounded-full bg-secondary"><div className="h-3 w-3 rounded-full bg-primary"></div></div>
                                <div className="pl-8">
                                    <div className="flex items-center justify-between">
                                        <p className="font-semibold">{log.action.replace(/_/g, ' ')} by {log.user}</p>
                                        <time className="text-xs text-muted-foreground">{log.timestamp ? format(new Date(log.timestamp), 'PPp') : 'N/A'}</time>
                                    </div>
...
export function ReviewBidDialog({ requisitionId, isOpen, onClose }: { requisitionId: string; isOpen: boolean, onClose: (refresh: boolean) => void; }) {
  const { toast } = useToast();
  const { user } = useAuth();
  ...
  return (
    <Dialog open={isOpen} onOpenChange={(open) => !open && onClose(false)}>
        <DialogContent className="max-w-4xl h-[95vh] flex flex-col">
            <DialogHeader>
                <DialogTitle>Review Award Recommendation</DialogTitle>
                <DialogDescription>
                    Review the details for requisition <span className="font-semibold text-primary">{requisitionId}</span> and make a final decision.
                </DialogDescription>
            </DialogHeader>
             <div className="flex-1 overflow-y-auto pr-6 -mr-6">
                ...
                : !requisition ? <div className="text-center p-8">Requisition not found or you do not have permission to view it.</div>
