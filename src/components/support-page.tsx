...
function ProcurementOfficerView({ tickets, onTicketUpdated }: { tickets: SupportTicket[], onTicketUpdated: () => void }) {
  const [selectedTicket, setSelectedTicket] = useState<SupportTicket | null>(null);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const { toast } = useToast();
  ...
  useEffect(() => {
    if (selectedTicket) {
      const saved = localStorage.getItem(`support-response-${selectedTicket.id}`);
      form.reset({ response: saved || selectedTicket.response || '', status: selectedTicket.status.replace(/ /g, '_') as 'In_Progress' | 'Closed' });
    }
  }, [selectedTicket, form]);
  ...
  const handleRespond = async (values: z.infer<typeof responseFormSchema>) => {
    ...
    try {
      const res = await fetch(`/api/support/${selectedTicket.id}`, {
        method: 'PATCH',
        headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${token}` },
        body: JSON.stringify({ response: values.response, status: values.status }),
      });
      if (!res.ok) throw new Error('Failed to submit response');
      toast({ title: 'Response Sent', description: 'Vendor has been notified.' });
      onTicketUpdated();
      setSelectedTicket(null);
    } catch (e) {
      toast({ variant: 'destructive', title: 'Error', description: 'Could not send response.' });
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <CardTitle>Vendor Questions Assigned To You</CardTitle>
          <CardDescription>Respond to tickets sent directly to you by vendors.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {Array.isArray(tickets) ? tickets.map(t => (
                <TableRow key={t.id}>
                  ...
                  <TableCell><Badge variant={t.status === 'Closed' ? 'default' : 'secondary'}>{t.status.replace(/_/g, ' ')}</Badge></TableCell>
                  <TableCell>{formatDistanceToNow(new Date(t.createdAt), { addSuffix: true })}</TableCell>
                  <TableCell className="text-right"><Button variant="outline" size="sm" onClick={() => setSelectedTicket(t)}>View & Respond</Button></TableCell>
                </TableRow>
              )) : null}
            </TableBody>
          </Table>
          ...
          <Form {...form}>
            <form onSubmit={form.handleSubmit(handleRespond)}>
              ...
              <div className="py-4 space-y-4">
                ...
                <FormField control={form.control} name="response" render={({ field }) => (
                  <FormItem><FormLabel>Your Response</FormLabel><FormControl><Textarea rows={6} placeholder="Type your response to the vendor..." {...field} /></FormControl><FormMessage /></FormItem>
                )} />
                <FormField control={form.control} name="status" render={({ field }) => (
                  <FormItem><FormLabel>Update Status</FormLabel><FormControl>
                    <select {...field} className="w-full p-2 border rounded-md bg-background">
              ...
              <DialogFooter>
                <DialogClose asChild><Button variant="ghost">Cancel</Button></DialogClose>
                <Button type="submit" disabled={isSubmitting}>{isSubmitting && <Loader2 className="mr-2 h-4 w-4 animate-spin" />}Submit Response</Button>
              </DialogFooter>
            </form>
          </Form>
        </DialogContent>
...
function SubmitTicketForm({ onTicketSubmitted }: { onTicketSubmitted: () => void }) {
  const { user, token } = useAuth();
  const { toast } = useToast();
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [recipientType, setRecipientType] = useState<'Admin' | 'ProcurementOfficer'>('Admin');
  const [procurementOfficers, setProcurementOfficers] = useState<Array<{ id: string; name: string }>>([]);
  const [selectedProcurementOfficerId, setSelectedProcurementOfficerId] = useState<string | null>(null);
  ...
  const fetchMyTickets = useCallback(async () => {
    if (!token) return;
    try {
      const res = await fetch('/api/support?scope=mine', { headers: { Authorization: `Bearer ${token}` } });
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!mounted) return;
        const officers = (all || []).filter((u: any) => Array.isArray(u.roles) && u.roles.some((r: any) => (r.name || r) === 'Procurement_Officer'))
          .map((u: any) => ({ id: u.id, name: u.name }));
        setProcurementOfficers(officers);
        if (officers.length > 0) setSelectedProcurementOfficerId(officers[0].id);
      } catch (e) {
        console.error('Failed to fetch procurement officers', e);
      }
    })();
  ...
  const onSubmit = async (values: z.infer<typeof ticketFormSchema>) => {
    ...
    setIsSubmitting(true);
    try {
      const payload: any = { ...values, requesterId: user.id };
      if (recipientType === 'ProcurementOfficer') {
        payload.recipientType = 'ProcurementOfficer';
        payload.procurementOfficerId = selectedProcurementOfficerId;
      } else {
      ...
      toast({ title: 'Ticket Submitted', description: 'Your ticket was created.' });
      form.reset();
      onTicketSubmitted();
    } catch (e) {
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Submit an issue for Admin or a Procurement Officer.</CardDescription>
        </CardHeader>
        <CardContent>
          <Form {...form}>
            <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <div className="flex gap-4 items-center">
                  <label className="flex items-center gap-2"><input type="radio" name="recipient" checked={recipientType === 'Admin'} onChange={() => setRecipientType('Admin')} /> <span className="ml-2">Admin</span></label>
                  <label className="flex items-center gap-2"><input type="radio" name="recipient" checked={recipientType === 'ProcurementOfficer'} onChange={() => setRecipientType('ProcurementOfficer')} /> <span className="ml-2">Procurement Officer</span></label>
                </div>
                {recipientType === 'ProcurementOfficer' && (
                  <div className="mt-2">
                    <Label>Select Procurement Officer</Label>
                    <select value={selectedProcurementOfficerId || ''} onChange={(e) => setSelectedProcurementOfficerId(e.target.value)} className="w-full p-2 border rounded-md bg-background">
                      {procurementOfficers.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                    </select>
                  </div>
                )}
              </div>
      ...
      <Card className="mt-4">
        ...
        <CardContent className="space-y-4">
          {myTickets && myTickets.length > 0 ? myTickets.map(ticket => (
            <Card key={ticket.id} className="p-4">
              ...
              {ticket.response && (
                <div className="mt-3 pt-3 border-t">
                  <p className="text-sm font-semibold text-primary">{ticket.procurementResponder?.name ? 'Procurement Officer Response' : (ticket.admin?.name ? 'Admin Response' : 'Response')}</p>
            ...
            <div className="text-center py-6 text-muted-foreground">
              <p className="mt-2">You haven't submitted any tickets yet.</p>
            </div>
          )}
        </CardContent>
      </Card>
...
function UserView({ tickets, onTicketSubmitted }: { tickets: SupportTicket[], onTicketSubmitted: () => void }) {
  ...
  // fetch procurement officers for selection when vendor wants to target a procurement officer
  useEffect(() => {
    let mounted = true;
    (async () => {
      try {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        console.error('Unexpected tickets payload from /api/support', data);
        setTickets([]);
      } else {
        setTickets(data);
      }
  ...
  return (
    <div>
      {role === 'Admin' ? (
        ...
        <div className="grid md:grid-cols-2 gap-8">
          <div>
            <ProcurementOfficerView tickets={tickets} onTicketUpdated={fetchTickets} />
          </div>
          <div>
            <SubmitTicketForm onTicketSubmitted={fetchTickets} />
          </div>
        </div>
      ) : (
